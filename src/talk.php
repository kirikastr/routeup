<!DOCTYPE html>
<html lang="en">

<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Talk</title>
	<!-- Links -->
	<link rel="stylesheet" href="./css/output.css">
</head>

<body>
	<div class="flex">
		<!-- Left bar -->
		<div class="relative w-[89px] bg-[#1675CA] pt-4" style="height: 120vh;">
			<!-- top -->
			<a href="index.php"><img src="./assets/logo.svg" class="w-[60px] mx-[15px]" alt=""></a>
			<hr class="w-[56px] mx-[17px] border-t-[3px] mt-6 opacity-25">
			<a href="index.php"><img src="./assets/icon/home.svg" class="w-[40px] mx-[25px] mt-[22px]" alt=""></a>
			<a href="talk.php"><img src="./assets/icon/speech.svg" class="w-[40px] mx-[25px] mt-[47px]" alt=""></a>
			<a href="reg.php"><img src="./assets/icon/people.svg" class="w-[40px] mx-[25px] mt-[47px]" alt=""></a>
			<a href="map.php"><img src="./assets/icon/map.svg" class="w-[40px] mx-[25px] mt-[47px]" alt=""></a>
			<!-- bottom -->
			<section class="absolute bottom-0 pb-[31px]">
				<hr class="w-[56px] mx-[17px] border-t-[3px] mb-[23px] opacity-25">
				<a href="index.php"><img src="./assets/icon/leave.svg" class="w-[40px] mx-[25px]" alt=""></a>
			</section>
		</div>

		<!-- Right bar -->
		<div>
			<!-- Header -->
			<div class="bg-[#18578A] h-[99px] flex relative" style="width: 1779px;">
				<div class="w-[612px] h-[72px] rounded-[25px] bg-white ml-6 mt-[13px] flex relative">
					<svg class="mt-[21px] ml-[17px]" width="30" height="30" viewBox="0 0 30 30" fill="none"
						xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
						<rect opacity="0.75" width="30" height="30" fill="url(#pattern0)" />
						<defs>
							<pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
								<use xlink:href="#image0_103_253" transform="scale(0.00195312)" />
							</pattern>
							<image id="image0_103_253" width="512" height="512"
								xlink:href="data:image/png;base64," />
						</defs>
					</svg>
					<div style="border: 1px solid black;" class="h-[48px] mt-3 ml-4 opacity-25"></div>
					<h1 class="text-2xl font-light ml-[10px] mt-4">Сообщества</h1>
					
				</div>
			</div>
			<div class="w-[1268px] mx-auto h-[981px]">
				<div class="w-[1268px] h-[171px] pl-10 pt-[9px] bg-[#0E283F]">
					<h1 class="text-[32px] font-light text-white">Сортировка</h1>
					<hr class="w-[1186px] border-black opacity-50 mt-[6px]">
					<section class="flex">
						<div class="flex">
							<h1 class="text-2xl font-normal text-white mt-[10px]">Город</h1>
							<label for="" class="ml-[10px] mt-[12px]">
								<img src="./assets/icon/attributes.svg" class="w-[31px]" alt="">
							</label>
						</div>
						<div class="flex ml-[80px]">
							<h1 class="text-2xl font-normal text-white mt-[10px]">Количество подписчиков</h1>
							<label for="" class="ml-[10px] mt-[12px]">
								<img src="./assets/icon/attributes.svg" class="[31px]" alt="">
							</label>
						</div>
					</section>
					<section class="flex">
						<div class="flex">
							<h1 class="text-2xl font-normal text-white mt-[10px]">А-Я</h1>
							<label for="" class="ml-[33px] mt-[12px]">
								<img src="./assets/icon/attributes.svg" class="w-[31px]" alt="">
							</label>
						</div>
						<div class="flex ml-[80px]">
							<h1 class="text-2xl font-normal text-white mt-[10px]">Активность</h1>
							<label for="" class="ml-[160px] mt-[12px]">
								<img src="./assets/icon/attributes.svg" class="w-[31px]" alt="">
							</label>
						</div>
					</section>
				</div>
				<div class="w-[1167px] h-[150px] mt-[26px] ml-[53px] flex relative">
					<img src="./assets/icon/logo_1.svg" class="w-[150px]" alt="">
					<div class="ml-7">
						<h1 class="font-light font-inter text-[40px]">Ориентировщики Якутии</h1>
						<p class="text-[16px] font-light font-inter opacity-75">269 подписчиков</p>
						<p class="text-[16px] font-light font-inter max-w-[754px]">Описание</p>
					</div>
					<button
						class="w-[208px] h-[50px] bg-[#D9D9D9] rounded-[50px] absolute right-0 top-[34px]">Подписаться</button>
				</div>
				<hr class="w-[1186px] mx-auto mt-6 opacity-50 border-t-[1px] border-black">
				<div class="w-[1167px] h-[150px] mt-[26px] ml-[53px] flex relative">
					<img src="./assets/icon/logo_2.svg" class="w-[150px]" alt="">
					<div class="ml-7">
						<h1 class="font-light font-inter text-[40px]">Sport Бazа</h1>
						<p class="text-[16px] font-light font-inter opacity-75">Количество подписчиков</p>
						<p class="text-[16px] font-light font-inter max-w-[754px]">Спортивный клуб «SportБАZA» - это не
							просто клуб
							или отдельные студии, это яркий и интересный проект со своей индивидуальностью и атмосферой.
							Совершенно новый формат не только для занятий спортом, но и место для общения и встреч с
							друзьями.</p>
					</div>
					<button
						class="w-[208px] h-[50px] bg-[#D9D9D9] rounded-[50px] absolute right-0 top-[34px]">Подписаться</button>
				</div>
				<hr class="w-[1186px] mx-auto mt-6 opacity-50 border-t-[1px] border-black">
				<div class="w-[1167px] h-[150px] mt-[26px] ml-[53px] flex relative">
					<img src="./assets/icon/logo_3.svg" class="w-[150px]" alt="">
					<div class="ml-7">
						<h1 class="font-light font-inter text-[40px]">Российский Азимут</h1>
						<p class="text-[16px] font-light font-inter opacity-75">Количество подписчиков</p>
						<p class="text-[16px] font-light font-inter max-w-[754px]">Российский Азимут» — одно из самых
							масштабных и
							многочисленных соревнований по спортивному ориентированию в мире. Оно сродни таким
							популярным спортивно-массовыми мероприятиям, как «Лыжня России» и «Кросс Наций».</p>
					</div>
					<button
						class="w-[208px] h-[50px] bg-[#D9D9D9] rounded-[50px] absolute right-0 top-[34px]">Подписаться</button>
				</div>
				<hr class="w-[1186px] mx-auto mt-6 opacity-50 border-t-[1px] border-black">
				<div class="w-[1167px] h-[150px] mt-[26px] ml-[53px] flex relative">
					<img src="./assets/icon/logo_4.svg" class="w-[150px]" alt="">
					<div class="ml-7">
						<h1 class="font-light font-inter text-[40px]">Федерация велосипедного спорта</h1>
						<p class="text-[16px] font-light font-inter opacity-75">Количество подписчиков</p>
						<p class="text-[16px] font-light font-inter max-w-[754px]">Привет и добро пожаловать в
							официальный аккаунт
							Федерации велосипедного спорта России! Это сообщество для всех — от новичка, только
							погружающегося в велокультуру, до настоящего мастера велоспорта.
						</p>
					</div>
					<button
						class="w-[208px] h-[50px] bg-[#D9D9D9] rounded-[50px] absolute right-0 top-[34px]">Подписаться</button>
				</div>
			</div>
		</div>
	</div>
</body>

</html>